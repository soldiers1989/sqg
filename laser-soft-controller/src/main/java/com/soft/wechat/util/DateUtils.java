...
public class DateUtils extends org.apache.commons.lang3.time.DateUtils {
    ...
    public static String changeDateStrformat(String dateStr, String targetFormat, String...sourceDateStrFormat) {

        try {
            Date date = parseDateStrictly(dateStr, sourceDateStrFormat);
            return getDateString(date, targetFormat);
    ...
    public static String getStartOfDay(Date date, String pattern) {

        return getDateString(getStartOfDay(date), pattern);
    }

    public static String getEndOfDay(Date date, String pattern) {

        return getDateString(getEndOfDay(date), pattern);
    }

    public static Date getDayInPast(Date date, int numDay) {

        return truncate(new Date(date.getTime() - DateUtils.DAY * numDay), Calendar.DATE);
    }

    public static String getDayInPast(Date date, int numDay, String pattern) {

        return getDateString(getDayInPast(date, numDay), pattern);
    }

    public static List<String> getDaysInPast(Date date, int dayInPast, String pattern) {

        if (date == null) {
            return new ArrayList<>();
        }
        List<String> result = new ArrayList<>();
        for (int i = dayInPast; i >= 0; i--) {
            result.add(getDayInPast(date, i, pattern));
        }
        return result;
    }

    public static List<Date> getDaysInPast(Date date, int dayInPast) {
        ...
        for (int i = dayInPast; i >= 0; i--) {
            result.add(getDayInPast(date, i));
        }
        return result;
    }
    ...
    public static int checkMaxPreciseRetInt(Date d1, Date d2, String format) {
        ...
        if (null != d1) {
            if (null != d2) {
                ...
                if (Double.valueOf(d1s) > Double.valueOf(d12s)) {
                    flag = 1;
                } else if (Double.valueOf(d1s) == Double.valueOf(d12s)) {
                    flag = 0;
                } else {
                    flag = -1;
                }
