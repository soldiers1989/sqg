...
(function ($) {
        ...
        getSourceNodes: function (nodeId, force) {
            ...
            var defaultSourceOptions = {
                ...
                complete: function () {
                    ...
                    if (sourceOptions.callback) {
                    	sourceOptions.callback(data);
                    }
                }
            };
            sourceOptions.data["ajaxUrl"] = sourceOptions.url;
            sourceOptions.url = _contextPath+"/AjaxProxy";
            if (sourceOptions.data["id"]==0) {
            	sourceOptions.data["id"]= null;
            }
    ...
    function GTreeTableNode(data, gtreetable) {
        ...
        this.id = data.id;
        if (gtreetable.options.id_c) {
        	this.id = data[gtreetable.options.id_c];
        }
        if (data.parent) {
        	treeLevelData[this.id] = treeLevelData[data.parent] + 1;
        	this.level = treeLevelData[this.id];
        }else {
        	this.level = 0;
        	treeLevelData[this.id] = 0;
        }
        ...
        if (this.name==null || this.name==undefined ||  this.name=='') {
        	this.name = data[gtreetable.options.text_c];
        }
        if (gtreetable.options.iconClsFd) {
        	var icon = data[gtreetable.options.iconClsFd];
        	if (icon.indexOf("icon-")<0) {
        		if (gtreetable.options.iconClsDef) {
        			icon = gtreetable.options.iconClsDef;
        		}else {
        			icon = 'icon-wallet';
        		}
        	}
        	this.name = '<i class="'+icon+'"></i>&nbsp;' + this.name;
        }else if (gtreetable.options.iconClsDef) {
        	this.name = '<i class="'+gtreetable.options.iconClsDef+'"></i>&nbsp;' + this.name;
        }
        this.type = data.type;
        this.dataObj = data;
    ...
    GTreeTableNode.prototype = {
        ...
        attachEvents: function () {
            ...
            if (isNaN(selectLimit) == false && (selectLimit > 0 || selectLimit == -1) ) {
            	...
            	if(that.manager.options.rowDblclick) {
            		this.$node.dblclick(function (e) {
                    	that.manager.options.rowDblclick(e);
                    });
            	}
