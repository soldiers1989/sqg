...
(function($) {
    ...
    var data_result = new Object();
    var data_result_down = new Object();
    // public methods
    var methods = {
        ...
        init: function(options) {
            ...
            return this.each(function() {
                 ...
                 * search
                 */
                var searchObj = $("#" + this.id + "_search");
                if (searchObj.attr("id") == undefined){
                	searchObj = $("#bs_searchForm");
                }
                var searchHtml = searchObj.html();
                var resetPro = "";
	           	 if(searchHtml == "" || searchHtml == undefined || searchHtml == null){
	           		 for(var i in settings.searching) {
	           			 var search_name = get_searching_name(settings.searching[i]);
	           			 if (parseInt(i)%3==0){
	           				 if(parseInt(i)!== 0){
	           					 searchHtml += "</div>";
	           				 }
	           				 searchHtml += "<div class='form-group '>";
	           			 }
	           			 var searchField = settings.searching[i]["field"];
	           			 var activeField = settings.searching[i]["activeField"];
	           			 var searchType = settings.searching[i]["type"];
	           			 var onfocus = settings.searching[i]["onfocus"];
	           			 resetPro = resetPro + searchField;
	           			 if(activeField != "" && activeField != null && activeField != undefined ){
	           				 if(onfocus != "" && onfocus != null && onfocus != undefined){
	           					onfocus += "('" + activeField + "','" + searchField + "')";
	           				 }
	           				 searchHtml += "<input type='hidden' name='" + activeField + "' id='search_" + activeField + "'>";
	           				resetPro = resetPro + "@" + activeField;
	           			 }
	           			 resetPro += ",";
	           			 searchHtml += "<label class='col-md-1 control-label' style='width:13%;padding-right: 10px;text-align: right;margin-top: 6px;'>"+search_name + "</label>";
	           			 searchHtml += "<div class='col-md-3' style='width:20%'>";
	           			 if(searchType == "select"){
	           				 // 下拉选择
	           				searchHtml += "<div class='input-group'>"
	           				 			+"<select class='form-control' name='" + searchField + "' id='search_" + searchField + "'></select>"
	           				 			+ "<span class='input-group-addon' style='cursor: pointer;' onclick='clearThis(\"search_" + searchField + "\")'><i class='fa fa-cut'></i></span></div>";
	           			 }else if(searchType == "date"){
	           				 // 日期类型
	           				searchHtml += "<div class='input-group'>";
	           				searchHtml += "<input type='search' class='form-control' style='width:103px!important' readonly='readonly' onclick=\"WdatePicker();\" name='"+searchField+"' id='search_" + searchField + "'>";
	           				var tofield = settings.searching[i]["tofield"];
	           				if(tofield != "undefined" && tofield != undefined){
	           					searchHtml += "<span style='padding: 4px 5px;display: table-cell;vertical-align: middle;'> - </span>" +
	           							"<input type='search' class='form-control' style='width:103px!important' readonly='readonly' onclick=\"WdatePicker();\" name='"+tofield+"' id='search_" + tofield + "'>";
	           					resetPro = resetPro + tofield + ",";
	           				}
	           				searchHtml += "</div>";
	           			 }else{
	           				 if(onfocus != "undefined" && onfocus != undefined){
	           					 // 弹出搜索页面
	           					 var inputStr = "<input type='search' placeholder='请选择' class='form-control' onfocus=\"" + onfocus + "\" name='"+searchField+"' id='search_" + searchField + "'>";
	           					 searchHtml += "<div class='input-group'>" + inputStr
	           						+ "<span class='input-group-addon' style='cursor: pointer;' onclick=\"" + onfocus + "\"><i class='fa fa-search'></i></span>"
           							+ "</div>";
	           				 } else{
	           					searchHtml += "<input type='search' class='form-control' name='"+searchField+"' id='search_" + searchField + "'>";
	           				 }
	           				 
	           			 }
	           			 searchHtml += "<span class='help-block'></span></div>";
	           			 
	           		 }
	           		 if (settings.searching.length%3==0){
	           			 searchHtml += "</div>";
	       			 }
	           		 searchHtml += "<div class='pull-right' style='margin-right: 2.7%;'>" +
					        			 "<button class='btn green  margin-bottom' onclick='" + settings.search_name + "();'><i class='fa fa-search'></i> 查 询</button>" +
					        			 "&nbsp;&nbsp;<button class='btn green filter-cancel' onclick='resetsearch(\""+resetPro+"\")'><i class='fa fa-refresh'></i> 重 置</button>" +
					        		"</div>";
					 searchHtml += "</div>";
...
//                elem_html += '<div id="' + tools_id + '" class="' + settings.toolsClass + '"></div>';
                ...
                elem_html += '<div id="' + no_results_id + '" class="' + settings.noResultsClass + '">没有记录</div>';
                            ...
                            switch(sel_index) {
                                case 0:
                                    $(selector_table_tr).each(function() {
                                        row_id = $(this).attr("id").substr(row_prefix_len);
                                        idx = methods.selectedRows.call(elem, "selected_index", row_id);
                                        if(idx == -1) {
                                            methods.selectedRows.call(elem, "add_id", row_id);
                                        }
                ...
                // PAGINATION --------------------------------------------------
                $.when(grid_init).then(function(dataobj, textStatus, jqXHR) {

                	var data = dataobj;
                	if (dataobj.success) {
                		data = dataobj.data;
                	}
                	if (settings.callbackDataFunction) {
                		data = settings.callbackDataFunction(dataobj);
                	}
                    var total_rows = data["total"];
        ...
        selectedRows: function(action, id) {
            ...
            switch(action) {
                case "get_ids":
                    ...
                    break;
                case "get_obj":
                	return data_result[id];
                    break;
                case "get_all_obj":
                	return data_result;
                    break;
                case "get_obj_down":
                	return data_result_down[id];
                    break;
                case "clear_all_ids":
                    s.selected_ids = [];
                    break;
        ...
        displayGrid: function(refresh_pag) {
            ...
            if(s.page) {
            	postData[s.page]=s.pageNum;
            }
            if(s.rows) {
            	postData[s.rows]=s.rowsPerPage;
            }
            var searchObj = $("#" + container_id + "_search");
            if (searchObj.attr("id") == undefined){
            	searchObj = $("#bs_searchForm");
            }
            for(var i in s.searching) {
            	var field_value;
            	var field_name = s.searching[i]["activeField"];
            	if(field_name == null || field_name == undefined || field_name == ""){
            		field_name = s.searching[i]["field"];
            	}
            	var tofield = s.searching[i]["tofield"];
            	if(tofield != null && tofield!=undefined && tofield != ""){
            		var tofieldValue = searchObj.find("#search_"+tofield).val();
            		if(tofieldValue != null && tofieldValue != undefined && tofieldValue != ""){
            			postData[tofield]=tofieldValue;
            		}
            	}
            	field_value = searchObj.find("#search_"+field_name).val();
            	if(field_value == null || field_value == undefined || field_value == "")
            		continue;
            	serachParam += serachParam == "" ? "?" : "&";
      			serachParam += field_name + "=" + field_value;
            ...
            var res = $.ajax({
                ...
                success: function(dataobj) {
                	...
                	if (s.callbackDataFunction) {
                		data = s.callbackDataFunction(dataobj);
                	}
                    ...
                    elem.data(pluginStatus)["total"] = total_rows;
                    ...
                    tbl_html = '<thead>';
                    tbl_html +="<input type='hidden' name='grid_total'   id='grid_total' value='" + total_rows + "'/>";
                	...
                	if("datagrid_down" == container_id){
                		data_result_down = new Object();
                	} else{
                		data_result = new Object();
                	}
                	var line = true;
                    for(row in page_data) {
                    	var data_row = page_data[row];
                    	var data_id = data_row[row_primary_key];
                    	if("datagrid_down" == container_id){
                    		data_result_down[""+data_id+""] = data_row;
                    	} else{
                    		data_result[""+data_id+""] = data_row;
                    	}
                        row_id_html = (row_primary_key ? ' id="' + table_id + '_tr_' + page_data[row][row_primary_key] + '"' : '');
                        if (!line) {
                        	tbl_html += '<tr' + row_id_html + ' style="background-color:#fcfdfc">';
                        	line = true;
                        }else {
                        	tbl_html += '<tr' + row_id_html + '>';
                        	line = false;
                        ...
                        for(i in s.columns) {
                            ...
                            if(column_is_visible(s.columns[i])) {
                            	if(s.columns[i].field == "" && s.columns[i].method != ""){
                            		var id = page_data[row][s.row_primary_key];
                            		tbl_html += "<td>";
                            		var methodParam = {"view":"fa fa-eye","edit":"fa fa-edit","del":"fa fa-trash-o","ck":"group-checkable","look":"icon-notebook","up":"fa fa-long-arrow-up","down":"fa fa-long-arrow-down","text":"form-control input-sm input-small input-inline"}
                            		var method = s.columns[i].method;
                            		for(var i=0;i<method.split(",").length;i++){
                            			var classStr =methodParam[method.split(",")[i]];
                            			if(method== "text"){ 
                              				tbl_html += '&nbsp;&nbsp;<input class="'+classStr+'" type="text" data-set="#sample_1 .checkboxes"  value="'+ page_data[row][s.columns[i].field]+'"  name="'+container_id+'"/>';
                              			} 
                            			else if(method== "ck"){ 
                            				tbl_html += '&nbsp;&nbsp;<input class="'+classStr+'" type="checkbox" data-set="#sample_1 .checkboxes"  value="'+id+'"  name="'+container_id+'"/>';
                            			}else{
                            				tbl_html += '&nbsp;&nbsp;<a href="javascript:;" class="' + classStr + '" onclick="' + container_id + '_' + method.split(",")[i] + '(\'' + id + '\')"></a>';
                            			}
                            		}
                            		tbl_html += "</td>";
                            	} else{ 
                            	 	if(s.columns[i].field != "" && s.columns[i].method == "text" ){
                            			var id = page_data[row][s.row_primary_key];
                                		tbl_html += "<td>";
                                		var methodParam = {"view":"fa fa-eye","edit":"fa fa-edit","del":"fa fa-trash-o","ck":"group-checkable","look":"icon-notebook","up":"fa fa-long-arrow-up","down":"fa fa-long-arrow-down","text":"form-control input-sm input-small input-inline"}
                                		var method = s.columns[i].method;
                                		for(var j=0;j<method.split(",").length;j++){
                                			var classStr =methodParam[method.split(",")[j]];
                                			if(method== "text"){ 
                                				var   name=s.columns[i].field+"_"+container_id;
                                  				tbl_html += '&nbsp;&nbsp;<input class="'+classStr+'" type="text" data-set="#sample_1 .checkboxes"  value="'+page_data[row][s.columns[i].field]+'"  id="'+id+'"  name="'+name+'" onchange="' + container_id + '_' + method.split(",")[j] + '(\'' + id + '\')"/>';
                                  			} 
                                		}
                                		tbl_html += "</td>";
                            		}else{ 
                            			if(s.columns[i].formatter) {
    	                          			tbl_html += '<td>' + s.columns[i].formatter(page_data[row]) + '</td>';
    	                          		}else {
    ...
    var get_searching_name = function(searching) {
        return searching.hasOwnProperty("searchingName") ? searching["searchingName"] : searching["field"];
    };
